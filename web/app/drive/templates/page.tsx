...
import { Download, Files, PencilLine, Trash2 } from "lucide-react";
...
export default function TemplatesPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await api<{ files: FileItem[] }>("/v1/files?view=templates");
      setFiles(r.files);
    } catch (e: any) {
    ...
    >
      ...
      {loading ? (
        ...
        <FileList
          ...
          actorLabel="You created"
          actions={(f) => [
            {
              label: "Open designer",
              icon: <PencilLine className="h-4 w-4" />,
              onClick: () =>
                f.templateId && router.push(`/templates/${f.templateId}/designer`),
            },
            {
              label: "Download",
          ...
          emptyState={
            <EmptyState
              ...
              description="Upload a PDF, create an HTML/Markdown template, or use the form builder — anything template-backed shows up here."
              action={
                <Button onClick={() => router.push("/drive")} size="sm">
                  Go to Drive
                </Button>
              }
            />
