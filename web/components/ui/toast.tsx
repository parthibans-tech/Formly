  ...
  "group pointer-events-auto relative flex w-full items-start gap-3 overflow-hidden rounded-lg border p-4 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        success:
          "border-success/40 bg-success/10 text-foreground dark:bg-success/15",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 text-foreground/60 opacity-0 transition-opacity hover:text-foreground focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 group-hover:opacity-100 group-[.destructive]:text-destructive-foreground",
      className
    )}
    toast-close=""
    {...props}
...
function ToastIcon({
  ...
  if (variant === "destructive")
    return <XCircle className="mt-0.5 h-5 w-5 shrink-0" aria-hidden />;
  if (variant === "success")
    return <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-success" aria-hidden />;
  return <Info className="mt-0.5 h-5 w-5 shrink-0 text-muted-foreground" aria-hidden />;
}
